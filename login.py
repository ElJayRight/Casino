...
class Window:
    ...
    def update_text(self):
        ...
        for i in range(len(self.box_locations)):
            pygame.draw.rect(self.screen,(255,255,255),(self.box_locations[i][0],
            self.box_locations[i][1],300,self.box_locations[i][3]))
    ...
    def create_user(self):
        if self.password==self.confirm_password and self.password!='':
            ...
            if count==0:
                file.write(self.username+' '+hashlib.sha256(bytes(self.password,'utf-8')).hexdigest()+' 500')
            else:
